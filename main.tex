...
Consider the expression \ct{XMLDOMParser parse: xmlString}. Evaluating this expression returns an abstract syntax tree describing the provided xml content. Evaluating this expression with an arbitrary XML content takes 156 ms on our machine\footnote{MacBook Pro, 8Gb of Ram, 2.26 GHz Intel Core 2 Duo}. 
...
Precisely determining the CPU time consumption of each of the method directly and indirectly involved is difficult. Most programming environment offers facilities to approximate this. We profiled the xml parsing using MessageTally, the standard profiler in Pharo, as follows:

\begin{lstlisting}
MessageTally spyOn: [ XMLDOMParser parse: xmlString ]
\end{lstlisting}

The total execution reported by MessageTally is 168 ms. We therefore see an increase of $(168 - 156) / 156 = 7\%$ of the execution time. Lengthening the execution time by 7\% for a time profile is a compromise acceptable in most of the situations in which Smalltalk is used (\ie non-realtime operation, with a relatively thin interaction with the operating system).
