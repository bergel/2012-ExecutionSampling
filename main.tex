...
We propose a statistical model to determine the right amount of iterations to reach a particle ration of reported methods \vp{, stablishing a metric for the quality of the profiler and helping to enhance it (I think, in the end, this is also a goal)}.
\end{abstract}
...
The research question addressed in this paper is: \emph{Can the number of multiple executions be related to the accuracy of the execution sampling?} \vp{Also quality? or accuracy includes this? What I mean is that some times we have a accurate time, but it's incomplete}
...
The total execution reported by MessageTally is 168 ms. We see an increase of $(168 - 156) / 156 = 7\%$ of the execution time. \vp{Sorry, but I do not understand this. I mean, this is a more accurate time? Or is an impact that as a concecuence of MessageTally?}%Lengthening the execution time by 7\% for a time profile is a compromise acceptable in most of the situations in which Smalltalk is used (\ie non-realtime operation, with a relatively thin interaction with the operating system).
...
By instrumenting the method, we get an accurate amount of methods involved in the execution. The average \vp{ratio between reported methods and executed methods} for our 8 applications (including the xml parsing) is 0.35. 

Merlin has the lowest ratio: only 1\% of the executed methods are reported by MessageTally. The reason is probably the extremely short execution of the unit tests (4 ms). Shout has the highest ratio (0.82). Shout has the longest execution time (6964 ms). \vp{Maybe it's more the repetition of the execution than the longer execution. Because we do not extend the execution time, we executed it more times. Maybe Shout has a bigger test suite, it executed more times some methods. Not sure if it's a huge difference, but I think it's more related to the sampling idea. Maybe specify is the profiling time than the methods execution time?}
...
\vp{Is it possible to add what each axis means? }
